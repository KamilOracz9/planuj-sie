...
/**
 * Business logic for Product that goes beyond straight CRUD - moved out of
 * ProductController per the project-architecture skill.
 */
class ProductService
{
    ...
    public static function priceBreakdown(string $locale, int $productId, ?int $channelId): array
    {
        if (!$channelId) {
            return ['channel_id' => null, 'currencies' => []];
        }
        ...
        $productAttributeOptions = static::priceableAttributeOptionsByAttribute('product', $productId, $locale);
        ...
        return ['channel_id' => $channelId, 'currencies' => $result];
    }
